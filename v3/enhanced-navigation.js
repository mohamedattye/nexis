(() => {
  ...
  function ensureUsersView() {
    ...
    if (nav && !nav.querySelector('[data-view="users"]')) {
      ...
      button.dataset.nexisAdminOnly = 'true';
      button.textContent = 'Utilisateurs';
      const reportsButton = nav.querySelector('[data-view="reports"]');
      if (reportsButton?.nextSibling) nav.insertBefore(button, reportsButton.nextSibling);
  ...
  function loadUsersModule() {
    loadScriptOnce('__NEXIS_USERS_LOADING__','users-module.js?v=20260728-users-1','Impossible de charger le module Utilisateurs.');
  }
  ...
  function setView(viewId, updateHash = true) {
    ...
    if (viewId === 'users' && window.NEXIS_AUTH?.isAdmin !== true) {
      viewId = 'dashboard';
    }
  ...
  document.addEventListener('nexis:auth-changed', (event) => {
    ...
    if (usersButton) usersButton.hidden = event.detail?.isAdmin !== true;
    if (event.detail?.isAdmin === true) loadUsersModule();
    if (location.hash === '#users' && event.detail?.isAdmin !== true) setView('dashboard');
  });
